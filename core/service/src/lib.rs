...
pub use components::{ServiceFactory, FullBackend, FullExecutor,
	Components, PoolApi, ComponentClient,
	ComponentBlock, FullClient, FullComponents,
...
//					}
		...
		let rpc = Components::RuntimeServices::start_rpc(
			...
			config.rpc_ws, config.rpc_cors.clone(), config.rpc_ws_max_connections , Arc::new(task_executor.clone()), transaction_pool.clone(),
			rpc_extensions,
		)?;
...
//			_offchain_workers: offchain_workers,
	...
	pub fn authority_key(&self) -> Option<primitives::ed25519::Pair> {
		...
		let tmp_keystore = self.keystore.clone();
		let keystore = tmp_keystore.read();
		keystore.public_keys::<keystore::ed25519::AppPublic>().ok().and_then(|pk_list| {
			pk_list.get(0).and_then(|p| {
				let d: Public = (*p).clone().into();
				keystore.key_pair_by_type::<Pair>(&d, primitives::crypto::key_types::ED25519).ok()
			})
		})
	}
...
fn start_rpc_servers<H: FnMut() -> rpc_servers::RpcHandler<rpc::Metadata>>(
	...
	Ok(Box::new((
		maybe_start_server(
			rpc_http,
			|address| rpc_servers::start_http(address, rpc_cors.as_ref(), gen_handler()),
		)?,
		maybe_start_server(
			rpc_ws,
