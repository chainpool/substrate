...
pub mod ext {
	...
	extern_functions! {
		...
		/// - `1` if the change trie root was found.
		/// - `0` if the change trie root was not found.
		fn ext_storage_changes_root2(parent_hash_data: *const u8, parent_hash_len: u32, result: *mut u8) -> u32;
...
impl CryptoApi for () {
	...
	fn secp256k1_ecdsa_verify(sig: &[u8], msg: &[u8; 32], pubkey: &[u8]) -> Result<bool, EcdsaVerifyError> {
		match unsafe {
			...
			2 => Err(EcdsaVerifyError::BadPublicKey),
			3 => Ok(false),
			_ => unreachable!("`secp256k1_ecdsa_verify` only returns 0, 1, 2 or 3; qed"),
		}
	}
