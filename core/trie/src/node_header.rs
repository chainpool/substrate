...
impl Decode for NodeHeader {
	fn decode<I: Input>(input: &mut I) -> Option<Self> {
		...
		match i & (0b11 << 6) {
			trie_constants::LEAF_PREFIX_MASK => decode_size(i, input).ok().map(NodeHeader::Leaf),
			trie_constants::BRANCH_WITHOUT_MASK => decode_size(i, input).ok().map(|r| NodeHeader::Branch(false, r)),
			trie_constants::BRANCH_WITH_MASK => decode_size(i, input).ok().map(|r| NodeHeader::Branch(true, r)),
			// do not allow any special encoding
			_ => None, // Err("Unallowed encoding".into()),
		}
	}
}
...
fn decode_size(first: u8, input: &mut impl Input) -> Result<usize, &'static str> {
	...
	while result <= trie_constants::NIBBLE_SIZE_BOUND {
		let n = input.read_byte().ok_or("read_bytes error")? as usize;
		if n < 255 {
