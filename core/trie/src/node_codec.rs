...
impl<H: Hasher> NodeCodecT<H> for NodeCodec<H> {
	...
	fn decode(data: &[u8]) -> rstd::result::Result<Node, Self::Error> {
		let input = &mut &*data;
		let head = NodeHeader::decode(input).ok_or("decode error")?;
		match head {
			...
			NodeHeader::Branch(has_value, nibble_count) => {
				...
				let value = if has_value {
					let count = <Compact<u32>>::decode(input).ok_or("decode error")?.0 as usize;
...
impl Bitmap {
	pub fn decode(data: &[u8]) -> Result<Self, Error> {
		Ok(Bitmap(u16::decode(&mut &data[..]).ok_or("decode error")?))
	}
