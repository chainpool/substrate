...
//! ## Overview
//!
//! This module extends accounts based on the `Currency` trait to have smart-contracts functionality. It can
//! be used with other modules that implement accounts based on `Currency`. These "smart-contracts accounts"
//! have the ability to instantiate smart-contracts and make calls to other contracts and non-contracts accounts.
//!
//! The smart-contracts code is stored once in a `code_cache`, and later retrievable via its `code_hash`.
//! This means that multiple smart-contracts can be instantiated from the same `code_cache`, without replicating
//! the code each time.
//!
//! When a smart-contracts is called, its associated code is retrieved via the code hash and gets executed.
//! This call can alter the storage entries of the smart-contracts account, instantiate new smart-contracts,
//! or call other smart-contracts.
//!
//! Finally, when an account is reaped, its associated code and storage of the smart-contracts account
//! will also be deleted.
//!
//! ### Gas
//!
//! Senders must specify a gas limit with every call, as all instructions invoked by the smart-contracts require gas.
//! Unused gas is refunded after the call, regardless of the execution outcome.
//!
//! If the gas limit is reached, then all calls and state changes (including balance transfers) are only
//! reverted at the current call's contracts level. For example, if contracts A calls B and B runs out of gas mid-call,
//! then all of B's calls are reverted. Assuming correct error handling by contracts A, A's other calls and state
//! changes still persist.
//!
//! ### Notable Scenarios
//!
//! Contract call failures are not always cascading. When failures occur in a sub-call, they do not "bubble up",
//! and the call will only revert at the specific contracts level. For example, if contracts A calls contracts B, and B
...
//! * `instantiate` - Deploys a new contracts from the given `code_hash`, optionally transferring some balance.
//! This instantiates a new smart contracts account and calls its contracts deploy handler to
...
use sr_primitives::{
	traits::{Hash, StaticLookup, Zero, MaybeSerializeDebug, Member},
...
use support::{
	Parameter, decl_module, decl_event, decl_storage, storage::child, StorageValue, StorageMap,
	parameter_types,
};
...
/// A function that generates an `AccountId` for a contracts upon instantiation.
...
impl<T: Trait> ContractInfo<T> {
	/// If contracts is alive then return some alive info
	...
	/// If contracts is alive then return some reference to alive info
	...
	/// If contracts is alive then return some mutable reference to alive info
	...
	/// If contracts is tombstone then return some tombstone info
	...
	/// If contracts is tombstone then return some reference to tombstone info
	...
	/// If contracts is tombstone then return some mutable reference to tombstone info
...
//	type Time: Time;
	...
	type Call: Parameter + Dispatchable<Origin=<Self as system::Trait>::Origin> + IsSubType<Module<Self>>;

	/// The overarching event type.
	type Event: From<Event<Self>> + Into<<Self as system::Trait>::Event>;

	/// A function type to get the contracts address given the instantiator.
	...
	/// Size of a contracts at the time of instantiation. This is a simple way to ensure
	...
	/// The amount of funds a contracts should deposit in order to offset
	/// the cost of one byte.
	///
	/// Let's suppose the deposit is 1,000 BU (balance units)/byte and the rent is 1 BU/byte/day,
	/// then a contracts with 1,000,000 BU that uses 1,000 bytes of storage would pay no rent.
	/// But if the balance reduced to 500,000 BU and the storage stayed the same at 1,000,
	/// then it would pay 500 BU/day.
	type RentDepositOffset: Get<BalanceOf<Self>>;

	/// Reward that is received by the party whose touch has led
	/// to removal of a contracts.
	...
	/// The fee required to instantiate a contracts instance.
	type ContractFee: Get<BalanceOf<Self>>;

	/// The base fee charged for calling into a contracts.
	type CallBaseFee: Get<Gas>;

	/// The base fee charged for instantiating a contracts.
...
/// Simple contracts address determiner.
///
...
//
//		/// Size of a contracts at the time of instantiaion. This is a simple way to ensure that
...
//
//		/// The fee required to instantiate a contracts instance. A reasonable default value
//		/// is 21.
//		const ContractFee: BalanceOf<T> = T::ContractFee::get();
//
//		/// The base fee charged for calling into a contracts. A reasonable default
//		/// value is 135.
//		const CallBaseFee: Gas = T::CallBaseFee::get();
//
//		/// The base fee charged for instantiating a contracts. A reasonable default value
//		/// is 175.
...
//		const BlockGasLimit: Gas = T::BlockGasLimit::get();
		...
		///
		/// * If the account is a smart-contracts account, the associated code will be
		...
		/// Instantiates a new contracts from the `codehash` generated by `put_code`, optionally transferring some balance.
		///
		/// Instantiation is executed as follows:
		///
		/// - The destination address is computed based on the sender and hash of the code.
		/// - The smart-contracts account is created at the computed address.
		/// - The `ctor_code` is executed in the context of the newly-created account. Buffer returned
		///   after the execution is saved as the `code` of the account. That code will be invoked
		///   upon any call received by this account.
		/// - The contracts is initialized.
		pub fn instantiate(
			origin,
		...
		/// Allows block producers to claim a small reward for evicting a contracts. If a block producer
		/// fails to do so, a regular users will be allowed to claim the reward.
		///
		/// If contracts is not evicted as a result of this call, no actions are taken and
		...
		fn claim_surcharge(origin, dest: T::AccountId, aux_sender: Option<T::AccountId>) {
			...
			// If poking the contracts has lead to eviction of the contracts, give out the rewards.
...
impl<T: Trait> Module<T> {
	/// Perform a call to a specified contracts.
	///
	...
	fn restore_to(
		...
		let mut origin_contract = <ContractInfoOf<T>>::get(&origin)
			...
			.ok_or("Cannot restore from inexisting or tombstone contracts")?;
		...
		let dest_tombstone = <ContractInfoOf<T>>::get(&dest)
			...
			.ok_or("Cannot restore to inexisting or alive contracts")?;
...
decl_event! {
		...
		<T as system::Trait>::Hash
	{
		...
		/// An event from contracts of account.
...
pub struct Schedule {
	...
	/// Base gas cost to call into a contracts.
	pub call_base_cost: Gas,

	/// Base gas cost to instantiate a contracts.
	...
	/// Maximum number of memory pages allowed for a contracts.
