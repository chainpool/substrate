...
//! Environment definition of the wasm smart-contracts runtime.
...
/// The value returned from ext_call and ext_instantiate contracts external functions if the call or
...
fn read_sandbox_memory_as<E: Ext, D: Decode>(
	...
	D::decode(&mut &buf[..]).ok_or(sandbox::HostError)
}
...
// a function set which can be imported by an executed contracts.
define_env!(Env, <E: Ext>,
	...
	// Make a call to another contracts.
	//
	// If the called contracts runs to completion, then this returns the status code the callee
	...
	// changes made by the called contracts are reverted. The scratch buffer is filled with the
	// output data returned by the called contracts, even in the case of a failure status.
	//
	// If the contracts traps during execution or otherwise fails to complete successfully, then
	// this function clears the scratch buffer and returns 0x0100. As with a failure status, any
	// state changes made by the called contracts are reverted.
	//
	// - callee_ptr: a pointer to the address of the callee contracts.
	...
	// Instantiate a contracts with the specified code hash.
	//
	// This function creates an account and executes the constructor defined in the code specified
	// by the code hash.
	//
	// If the constructor runs to completion, then this returns the status code that the newly
	// instantiated contracts returns on exit in the bottom 8 bits of the return value. The top 24
	// bits are 0s. A status code of 0 indicates success, and any other code indicates a failure.
	// On failure, any state changes made by the called contracts are reverted and the contracts is
	// not instantiated. On a success status, the scratch buffer is filled with the encoded address
	// of the newly instantiated contracts. In the case of a failure status, the scratch buffer is
	// cleared.
	//
	// If the contracts traps during execution or otherwise fails to complete successfully, then
	// this function clears the scratch buffer and returns 0x0100. As with a failure status, any
	...
	//
	// Returns 0 on the successful contracts instantiation and puts the address of the instantiated
	// contracts into the scratch buffer. Otherwise, returns non-zero value and clears the scratch
	// buffer.
	//
	...
	// extrinsic will be returned. Otherwise, if this call is initiated by another contracts then the
	// address of the contracts will be returned.
	ext_caller(ctx) => {
		ctx.scratch_buf.clear();
	...
	// Stores the address of the current contracts into the scratch buffer.
	ext_address(ctx) => {
		ctx.scratch_buf.clear();
	...
	// Record a request to restore the caller contracts to the specified contracts.
	//
	// At the finalization stage, i.e. when all changes from the extrinsic that invoked this
	// contracts are commited, this function will compute a tombstone hash from the caller's
	// storage and the given code hash and if the hash matches the hash found in the tombstone at
	// the specified address - kill the caller contracts and restore the destination contracts and set
	...
	// with the address of the to be restored contracts.
	// `code_hash_ptr`, `code_hash_len` - the pointer and the length of a buffer that encodes
	// a code hash of the to be restored contracts.
	...
	// Copy data from the scratch buffer starting from `offset` with length `len` into the contracts
	// memory. The region at which the data should be put is specified by `dest_ptr`.
	//
	// In order to get size of the scratch buffer use `ext_scratch_size`. At the start of contracts
	// execution, the scratch buffer is filled with the input data. Whenever a contracts calls
	...
	// Copy data from contracts memory starting from `src_ptr` with length `len` into the scratch
	...
	// Deposit a contracts event with the data buffer and optional list of topics. There is a limit
	...
	// Set rent allowance of the contracts
	//
	...
	// This function may be removed at any time, superseded by a more general contracts debugging feature.
	...
	// Stores the current block number of the current contracts into the scratch buffer.
