...
//!
//! - In order to run contracts code we need to instrument it with gas metering.
...
//! - Before running contracts code we check if the cached code has the schedule version that
...
//! this guarantees that every instrumented contracts code in cache cannot have the version equal to the current one.
//! Thus, before executing a contracts it should be reinstrument with new schedule.
