	...
	pub fn build(self) -> runtime_io::TestExternalities<Blake2Hasher> {
		...
		t.extend(
			balances::GenesisConfig::<Test> {
				...
				creation_fee: self.instantiation_fee,
				vesting: vec![],
			}
			.build_storage()
			.unwrap()
...
fn dispatch_call_not_dispatched_after_top_level_transaction_failure() {
	...
	with_externalities(
		...
		|| {
			...
			// Call the newly instantiated contracts. The contracts is expected to dispatch a call
			// and then trap.
			assert_err!(
				Contract::call(
					Origin::signed(ALICE),
...
/// if removes is true then assert that the contracts is a tombstonedead
...
fn call_removed_contract() {
	...
	with_externalities(
		...
		|| {
			...
			// Calling contracts should succeed.
			assert_ok!(Contract::call(Origin::signed(ALICE), BOB, 0, 100_000, call::null()));

			// Advance blocks
			System::initialize(&10, &[0u8; 32].into(), &[0u8; 32].into(), &Default::default());

			// Calling contracts should remove contracts and fail.
			assert_err!(
				Contract::call(Origin::signed(ALICE), BOB, 0, 100_000, call::null()),
				"contracts has been evicted"
			);

 			// Subsequent contracts calls should also fail.
			assert_err!(
				Contract::call(Origin::signed(ALICE), BOB, 0, 100_000, call::null()),
				"contracts has been evicted"
			);
...
fn default_rent_allowance_on_instantiate() {
	...
	with_externalities(
		...
		|| {
			...
			// Check contracts is still alive
	...
	with_externalities(
		...
		|| {
			...
			if test_different_storage || test_restore_to_with_dirty_storage {
				...
				// restoration contracts is still in place and also that `BOB` doesn't exist.
				assert!(ContractInfoOf::<Test>::get(BOB).unwrap().get_tombstone().is_some());
				...
				// contracts `DJANGO` ceased to exist and that `BOB` returned back.
...
fn storage_max_value_limit() {
	...
	with_externalities(
		...
		|| {
			...
			// Call contracts with allowed storage value.
			assert_ok!(Contract::call(
				Origin::signed(ALICE),
				BOB,
				0,
			...
			// Call contracts with too large a storage value.
			assert_err!(
				Contract::call(
					Origin::signed(ALICE),
					BOB,
...
fn deploy_and_call_other_contract() {
	...
	with_externalities(
		...
		|| {
			...
			// Call BOB contracts, which attempts to instantiate and call the callee contracts and
...
fn self_destruct_by_draining_balance() {
	...
	with_externalities(
		...
		|| {
			...
			// Check that the BOB contracts has been instantiated.
			assert_matches!(
...
// This tests that one contracts cannot prevent another from self-destructing by sending it
...
fn destroy_contract_and_transfer_funds() {
	...
	with_externalities(
		...
		|| {
			...
			// This deploys the BOB contracts, which in turn deploys the CHARLIE contracts during
			// construction.
			assert_ok!(Contract::instantiate(
				Origin::signed(ALICE),
				200_000,
			...
			// Check that the CHARLIE contracts has been instantiated.
			assert_matches!(
...
fn cannot_self_destruct_in_constructor() {
	...
	with_externalities(
		...
		|| {
			...
			// Fail to instantiate the BOB contracts since its final balance is below existential
			// deposit.
			assert_err!(
