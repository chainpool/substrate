...
pub enum RentOutcome {
	...
	/// * or account doesn't have a contracts,
...
fn try_evict_or_and_pay_rent<T: Trait>(
	...
	// How much block has passed since the last deduction for the contracts.
	let blocks_passed = {
	...
	// An amount of funds to charge per block for storage taken up by the contracts.
	let fee_per_block = {
	...
	if fee_per_block.is_zero() {
		// The rent deposit offset reduced the fee to 0. This means that the contracts
		// gets the rent for free.
		return (RentOutcome::Exempted, Some(ContractInfo::Alive(contract)));
	}

	// The minimal amount of funds required for a contracts not to be evicted.
	let subsistence_threshold = T::Currency::minimum_balance() + T::TombstoneDeposit::get();

	if balance < subsistence_threshold {
		// The contracts cannot afford to leave a tombstone, so remove the contracts info altogether.
	...
	if insufficient_rent || !can_withdraw_rent {
		// The contracts cannot afford the rent payment and has a balance above the subsistence
...
///
/// `handicap` gives a way to check or pay the rent up to a moment in the past instead
/// of current block. E.g. if the contracts is going to be evicted at the current block,
